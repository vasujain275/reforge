...
-- Using INSERT OR IGNORE to skip duplicates based on title uniqueness
-- Note: SQLite doesn't have a native UNIQUE constraint on patterns.title in the schema,
-- so we rely on INSERT OR IGNORE with a WHERE NOT EXISTS clause
